...
func _extract(root: String):
    ...
    for i in len(_paths):
        ...
        if err != OK:
            push_error("Could not open {0} for read, error {1}".format([fpath, err]))
            continue
...
func _finished():
    ...
    print(str("Extraction took ", elapsed, " seconds"))
...
#				# Multiline or procedural strings not supported
#				push_error("End bracket not found in {0}, line {1}".format([fpath, line_number]))
#				break
...
static func _walk(folder_path: String, file_action: FuncRef, filter: FuncRef):
    ...
    if err != OK:
        push_error("Could not open directory {0}, error {1}".format([folder_path, err]))
        return
