...
func add_key(key_value: String) -> void:
...
func save_data_remaps() -> void:
    if data_remaps.remapkeys.size() > 1 or (data_remaps.remapkeys.size() == 1 and data_remaps.remapkeys[0].remaps.size() > 0 and not data_remaps.remapkeys[0].remaps[0].value.empty()):
        _save_data_remaps()
